...
impl GifExporter {
    ...
    pub fn export_gif(frames: &[RecordedVideoFrame], fps: u32) -> Result<Vec<u8>, MediaError> {
        if frames.is_empty() {
            return Err(MediaError::EncodingFailed(
                "No frames to export to GIF".to_string(),
            ));
        }
        ...
        let mut output = Vec::new();
        {
            ...
            encoder.set_repeat(Repeat::Infinite).map_err(|e| {
                MediaError::EncodingFailed(format!("Failed to set GIF repeat: {:?}", e))
            })?;

            for f in frames {
                let img = RgbaImage::from_raw(f.width, f.height, f.rgba_data.clone()).ok_or_else(
                    || MediaError::EncodingFailed("Invalid frame buffer dimensions".to_string()),
                )?;

                let gif_frame = Frame::from_parts(img, 0, 0, delay);
                encoder.encode_frame(gif_frame).map_err(|e| {
                    MediaError::EncodingFailed(format!("Failed to encode GIF frame: {:?}", e))
                })?;
            }
        }
